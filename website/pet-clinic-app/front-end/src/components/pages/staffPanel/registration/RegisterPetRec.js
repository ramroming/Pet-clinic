...
const RegisterPetRec = () => {
  ...
  useEffect(() => {
    ...
    const registerPet = async () => {
      ...
      try {
        const parsedData = await sendRequest(`${process.env.REACT_APP_BACKEND_URL}receptionist/pets/`, 'POST', state.dataToSend, {
          'Authorization': `Bearer ${auth.token}`
        })
        if (parsedData && isMount) {
  ...
  }, [state.isLoading, auth.token, sendRequest, state.dataToSend, dispatch, setPageIsLoading])
  const submitFormPet = (e) => {
    e.preventDefault()
    dispatch({ type: 'validate', withUserName: true })
  }
    ...
    <>
      {state.responseData &&
        <Modal
          modalClass='success'
          header='Success!!'
          body={'Pet has been registered successfully'}
          dispatch={dispatch}
          refresh={true}
        />}
      <h4>Registration</h4>
